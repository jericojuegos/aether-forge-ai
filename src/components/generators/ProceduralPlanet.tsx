...
const PlanetShaderMaterial = {
  ...
  `,
    ...
    void main() {
      ...
      vec3 color = mix(uColorWater, uColorLand, step(uWaterLevel, vElevation + 0.5)); // +0.5 to normalize roughly
...
export const ProceduralPlanet = () => {
    ...
    useFrame((state) => {
        if (materialRef.current) {
            ...
            materialRef.current.uniforms.uColorWater.value.set(planetConfig.colorWater);
            materialRef.current.uniforms.uColorLand.value.set(planetConfig.colorLand);
            materialRef.current.uniforms.uWaterLevel.value = planetConfig.waterLevel;
        }
        if (meshRef.current) {
    ...
    return (
        <group>
            <mesh ref={meshRef}>
                <icosahedronGeometry args={[2, planetConfig.detail]} />
                <shaderMaterial
                    ref={materialRef}
                    args={[PlanetShaderMaterial]}
                    transparent
                />
            </mesh>
